<template>
    <div v-if="permsArray.includes('goldtype_create') || permsArray.includes('admin')">
        <template v-if="customType == 'menu'">
            <a-menu-item @click="showAdd" key="goldtype">
                <template #icon>
                    <slot name="icon"></slot>
                </template>
                <slot></slot>
        ...
        <AddEdit
            ...
            :formData="formData"
            :data="formData"
            :pageTitle="$t('goldtype.add')"
            :successMessage="$t('goldtype.created')"
        />
    </div>
</template>
<script>
