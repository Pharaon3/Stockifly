<template>
    <AdminPageHeader>
        <template #header>
            <a-page-header :title="$t(`menu.goldtype`)" class="p-0" />
        </template>
        <template #breadcrumb>
        ...
        <a-row :gutter="[16, 16]">
            <a-col :xs="24" :sm="24" :md="12" :lg="10" :xl="10">
                <a-space>
                    <template
                        v-if="
                            permsArray.includes('goldtype_create') ||
                    ...
                    >
                        <a-space>
                            ...
                            <ImportGoldtype
                                :pageTitle="$t('goldtype.import_goldtype')"
                                :sampleFileUrl="sampleFileUrl"
                                importUrl="goldtype/import"
                                @onUploadSuccess="getGoldtype"
                            />
                        </a-space>
                    </template>
                    <a-button
                        v-if="
                            ...
                            (permsArray.includes('goldtype_delete') ||
                                permsArray.includes('admin'))
                        "
                        type="primary"
                    ...
                    >
                        <template #bodyCell="{ column, text, record }">
                            ...
                            <template v-if="column.dataIndex === 'action'">
                                <a-button
                                    v-if="
                                        permsArray.includes('goldtype_edit') ||
                                        permsArray.includes('admin')
                                    "
                                    type="primary"
...
import ImportGoldtype from "../../../../common/core/ui/Import.vue";
...
export default {
    ...
    setup() {
        ...
        const { t } = useI18n();
        const sampleFileUrl = window.config.goldtype_sample_file;
        ...
        const getGoldtype = () => {
            axiosAdmin
                .get(
                    `goldtype?fields=id,xid,slug,name&limit=10000`
                )
                .then((response) => {
                    const allGoldtypeArray = [];
                    var listArray = response.data;
                    // listArray = sortBy(listArray, "x_parent_id");

                    listArray.forEach((node) => {
                        // No parentId means top level
                        if (!node.x_parent_id) return allGoldtypeArray.push(node);
                    ...
                    allGoldtype.value = allGoldtypeArray;
                });
        };
        ...
        const editItem = (item) => {
            ...
            viewData.value = item;
            addEditUrl.value = `goldtype/${item.xid}`;
        ...
        const showDeleteConfirm = (id) => {
            Modal.confirm({
                ...
                content: t("goldtype.delete_message"),
                centered: true,
                okText: t("common.yes"),
                okType: "danger",
                cancelText: t("common.no"),
                onOk() {
                    axiosAdmin.delete(`goldtype/${id}`).then(() => {
                        getGoldtype();
                        notification.success({
                            message: t("common.success"),
                            description: t("goldtype.deleted"),
                        });
                    });
                },
                onCancel() {},
                ...
                onOk() {
                    ...
                    forEach(selectedRowKeys.value, (selectedRow) => {
                        allDeletePromise.push(
                            axiosAdmin.delete(`goldtype/${selectedRow}`)
                        );
                    });
