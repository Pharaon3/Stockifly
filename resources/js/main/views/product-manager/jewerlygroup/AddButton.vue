<template>
    <div v-if="permsArray.includes('jewerlygroup_create') || permsArray.includes('admin')">
        <template v-if="customType == 'menu'">
            <a-menu-item @click="showAdd" key="jewerlygroup">
                <template #icon>
                    <slot name="icon"></slot>
                </template>
                <slot></slot>
        ...
        <AddEdit
            ...
            :formData="formData"
            :data="formData"
            :pageTitle="$t('jewerlygroup.add')"
            :successMessage="$t('jewerlygroup.created')"
        />
    </div>
</template>
<script>
