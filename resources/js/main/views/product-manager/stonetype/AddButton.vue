<template>
    <div v-if="permsArray.includes('stonetype_create') || permsArray.includes('admin')">
        <template v-if="customType == 'menu'">
            <a-menu-item @click="showAdd" key="stonetype">
                <template #icon>
                    <slot name="icon"></slot>
                </template>
                <slot></slot>
        ...
        <AddEdit
            ...
            :formData="formData"
            :data="formData"
            :pageTitle="$t('stonetype.add')"
            :successMessage="$t('stonetype.created')"
        />
    </div>
</template>
<script>
